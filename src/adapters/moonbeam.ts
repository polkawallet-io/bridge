...
class BaseMoonbeamAdapter extends BaseCrossChainAdapter {
  ...
  public subscribeMaxInput (_: string, __: string, ___: ChainName): Observable<FN> {
    throw new ApiNotFound(this.chain.id);
  }

  public createTx (_: CrossChainTransferParams): SubmittableExtrinsic<'promise', ISubmittableResult> | SubmittableExtrinsic<'rxjs', ISubmittableResult> {
    throw new ApiNotFound(this.chain.id);
  }
}
